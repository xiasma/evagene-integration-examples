...
test('defaults base URL and label when not provided', () => {
  ...
  strictEqual(config.label, 'Family pedigree');
  strictEqual(config.nameSuffix, undefined);
});

test('accepts --name and --label', () => {
  const config = loadConfig(
    [VALID_UUID, '--name', 'aunt-lou', '--label', 'Lou family tree'],
    { EVAGENE_API_KEY: 'evg_test' },
  );

  strictEqual(config.nameSuffix, 'aunt-lou');
  strictEqual(config.label, 'Lou family tree');
});
...
test('--name without a value throws', () => {
  throws(
    () => loadConfig([VALID_UUID, '--name'], { EVAGENE_API_KEY: 'evg_test' }),
    ConfigError,
  );
});
