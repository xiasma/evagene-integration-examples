...
def _to_preview(family: ExtractedFamily) -> str:
    ...
    status = _format_status(proband.affection_status)
    lines.append(f"  proband  {proband.display_name} ({sex}{year}){status}")
...
def _format_relative(entry: RelativeEntry) -> str:
    header = f"{entry.display_name}{_format_year(entry.year_of_birth)}"
    header += _format_status(entry.affection_status)
    return _with_notes(header, entry.notes)
...
def _format_sibling(sibling: SiblingEntry) -> str:
    ...
    header += _format_status(sibling.affection_status)
...
def _format_status(status: AffectionStatus) -> str:
    if status is AffectionStatus.CLEAR:
        return ""
