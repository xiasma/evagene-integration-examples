...
from pedigree_ocr.extraction_schema import (
...
def test_affection_status_is_a_required_field_on_every_individual_shape() -> None:
    properties = build_tool_schema()["input_schema"]["properties"]

    assert "affection_status" in properties["proband"]["required"]
    assert "affection_status" in properties["mother"]["required"]
    assert "affection_status" in properties["maternal_grandmother"]["required"]
    assert "affection_status" in properties["siblings"]["items"]["required"]
...
def test_parse_sample_extraction_returns_expected_structure() -> None:
    ...
    assert family.proband.affection_status is AffectionStatus.CLEAR
    ...
    assert family.paternal_grandfather is not None
    assert family.paternal_grandfather.affection_status is AffectionStatus.AFFECTED
    assert len(family.siblings) == 1
    (alice,) = family.siblings
    assert alice.relation is SiblingRelation.SISTER
    assert alice.affection_status is AffectionStatus.AFFECTED
...
def test_rejects_unknown_affection_status() -> None:
    with pytest.raises(ExtractionSchemaError):
        parse_extraction(
            {
                "proband": {
