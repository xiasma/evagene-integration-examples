...
from nice_traffic_light.risk_api_client import ApiError, RiskApiClient
...
def test_posts_nice_model_to_risk_calculate() -> None:
    gateway = _RecordingGateway(
        _StubResponse(200, {"cancer_risk": {"nice_category": "near_population"}}),
    )

    payload = _client(gateway).calculate_nice(_PEDIGREE_ID)
    ...
    assert gateway.last_body == {"model": "NICE"}
    assert payload["cancer_risk"]["nice_category"] == "near_population"


def test_includes_counselee_when_provided() -> None:
    gateway = _RecordingGateway(_StubResponse(200, {}))

    _client(gateway).calculate_nice(_PEDIGREE_ID, counselee_id=_COUNSELEE_ID)
...
def test_raises_api_error_on_non_2xx() -> None:
    ...
    with pytest.raises(ApiError):
        _client(gateway).calculate_nice(_PEDIGREE_ID)
