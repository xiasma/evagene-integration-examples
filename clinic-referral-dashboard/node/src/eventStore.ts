/**
 * SQLite-backed, hash-chained audit log of webhook deliveries.
 *
...
export class EventStore {
  ...
  list(limit: number, offset: number): EventRow[] {
    const rows = this.db
      ...
      .all(limit, offset) as readonly RawEventRow[];
  ...
  verifyChain(): VerifyResult {
    ...
    let expectedPrev = '';
    for (const raw of stmt.iterate() as IterableIterator<RawEventRow>) {
