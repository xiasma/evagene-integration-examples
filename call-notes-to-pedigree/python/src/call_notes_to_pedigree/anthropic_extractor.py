"""Extract an :class:`ExtractedFamily` from a free-text transcript.

The ``AnthropicExtractor`` depends on an ``LlmGateway`` abstraction so
tests can substitute a fake that returns a canned tool-use payload.  The
