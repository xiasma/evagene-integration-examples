...
test('defaults base URL and output dir when env is unset', () => {
  ...
  strictEqual(config.outputDir, process.cwd());
  strictEqual(config.openBrowser, false);
});
...
test('parses --output-dir and --open flags', () => {
  const config = loadConfig([VALID_UUID, '--output-dir', '/tmp/x', '--open'], {
    EVAGENE_API_KEY: 'evg_test',
  });

  strictEqual(config.outputDir, '/tmp/x');
  strictEqual(config.openBrowser, true);
});
