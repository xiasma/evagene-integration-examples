...
def _submission(**overrides: Any) -> IntakeSubmission:
...
def test_proband_only_submission_performs_setup_then_stops() -> None:
    client = _RecordingClient()
    service = IntakeService(client)

    result = service.create(_submission())
...
def test_parents_added_before_respective_grandparents() -> None:
    client = _RecordingClient()
    service = IntakeService(client)
    ...
    add_relative_calls = [call for call in client.calls if call.op == "add_relative"]
    relatives = [
        (call.payload["args"].relative_type, call.payload["args"].display_name)
        for call in add_relative_calls
    ]
    assert relatives == [
        ("mother", "Grace"),
        ("father", "Henry"),
...
def test_grandparent_with_no_parent_is_skipped() -> None:
    ...
    service.create(_submission(maternal_grandmother=RelativeEntry(display_name="Edith")))

    assert not any(call.op == "add_relative" for call in client.calls)


def test_relatives_added_count_includes_every_add_relative() -> None:
    client = _RecordingClient()
    service = IntakeService(client)
    ...
    result = service.create(
        _submission(
            ...
            father=RelativeEntry(display_name="Henry"),
            maternal_grandmother=RelativeEntry(display_name="Edith"),
            maternal_grandfather=RelativeEntry(display_name="Cecil"),
            paternal_grandmother=RelativeEntry(display_name="Margaret"),
            paternal_grandfather=RelativeEntry(display_name="Arthur"),
            siblings=(
                SiblingEntry(
                    display_name="Alice",
    ...
    assert result.relatives_added == 7
