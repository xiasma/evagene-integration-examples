"""Thin client for the subset of the Evagene REST API used by the intake flow.

One method per endpoint; no orchestration (that lives in IntakeService).
Depends on an :class:`HttpGateway` so tests supply a fake.
"""
...
from .intake_submission import BiologicalSex
...
class AddRelativeArgs:
    pedigree_id: str
    relative_of: str
    relative_type: str  # matches Evagene's RelativeType enum (e.g. "mother", "sister")
    display_name: str
    biological_sex: BiologicalSex
    year_of_birth: int | None = None
...
class EvageneApi(Protocol):
    """Surface the intake service depends on; tests substitute their own implementation."""
