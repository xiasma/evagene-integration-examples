"""Orchestrates the Evagene REST calls that turn an :class:`IntakeSubmission`
into a persisted pedigree.

No HTTP knowledge -- depends only on :class:`EvageneApi`.  The order of
calls is load-bearing: grandparents need the parent individual to exist
before their ``relative_of`` can point at it.
"""
...
class IntakeService:
    ...
    def create(self, submission: IntakeSubmission) -> IntakeCreationResult:
        pedigree_id = self._client.create_pedigree(f"{submission.proband.display_name}'s family")
        proband_id = self._client.create_individual(
            CreateIndividualArgs(
                display_name=submission.proband.display_name,
                biological_sex=submission.proband.biological_sex,
                year_of_birth=submission.proband.year_of_birth,
            )
        )
        ...
        mother_id = self._maybe_add_relative(
            pedigree_id, proband_id, "mother", BiologicalSex.FEMALE, submission.mother
        )
        father_id = self._maybe_add_relative(
            pedigree_id, proband_id, "father", BiologicalSex.MALE, submission.father
        )
        ...
        if mother_id is not None:
            relatives_added += self._add_grandparents(
                pedigree_id,
                mother_id,
                submission.maternal_grandmother,
                submission.maternal_grandfather,
            )
        if father_id is not None:
            relatives_added += self._add_grandparents(
                pedigree_id,
                father_id,
                submission.paternal_grandmother,
                submission.paternal_grandfather,
            )
        relatives_added += self._add_siblings(pedigree_id, proband_id, submission.siblings)
