...
Mirrors the structure used by the ``call-notes-to-pedigree`` and
``family-history-intake-form`` demos so the writer can reuse the same
add-relative orchestration. Deliberately duplicated rather than
cross-imported to keep each demo runnable in isolation.
"""
