...
| `sample-transcript.txt` | Expected Whisper output for a short dictated family history. Used by the transcription-gateway fake in the test suite. Written to include typical speech disfluencies (filler words, a mid-sentence correction) to match what Whisper returns for spontaneous spoken input. | Hand-written, synthetic family. |
