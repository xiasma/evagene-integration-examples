...
class RiskApiClient:
    ...
    def calculate(
        ...
        *,
        ...
        """Run a single risk calculation for the given model."""
        ...
        if not _HTTP_OK_LOWER <= response.status_code < _HTTP_OK_UPPER:
            raise ApiError(
                f"Evagene API returned HTTP {response.status_code} for model {model}"
            )

        payload = response.json()
        if not isinstance(payload, dict):
            raise ApiError(
                f"Evagene API returned non-object JSON for model {model}: "
