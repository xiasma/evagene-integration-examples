...
|---|---|---|
| **R 4.3+** | `R -e 'install.packages(c("httr2","jsonlite","testthat"))'` | `Rscript inst/bin/bayesmendel-comparator.R <pedigree-id>` |
| **Python 3.11+** | `python -m venv .venv` · (activate) · `pip install -e .[dev]` | `python -m bayesmendel_comparator <pedigree-id>` |
